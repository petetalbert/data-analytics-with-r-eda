...
## How many rows are in these files?

Sometimes I will say records instead of rows and fields instead of columns.
...
## Why are there only 14 episodes in season 4?
...
## Working with median IMDB ratings

Here we see the median and mean are quality stats because they accurately depict the most common/typical imdb rating (there's a normal distribution).

```{r}
office_ratings_summary <- office_ratings %>% 
...
office_ratings %>%
  ...
  geom_vline(data = office_ratings_summary, aes(xintercept = avg_imdb_rating), linetype = 5) + # geom_vline()
  geom_vline(data = office_ratings_summary, aes(xintercept = median_imdb_rating), linetype = 6)


office_ratings_median <- office_ratings %>% 
  mutate(above_median = if_else(imdb_rating >= office_ratings_summary$median_imdb_rating, "Above median", "Below median"))


office_ratings_median %>% 
  mutate(above_median = fct_rev(above_median),
         season = as.factor(season)) %>% 
  count(season, above_median) %>% 
  ggplot(aes(x = season, y = n, fill = above_median)) +
  geom_col(position = "fill")


```

What proportion of lower IMDB ratings happened after season 8?

```{r}
office_ratings_median %>% 
  filter(above_median == "Below median") %>% 
  count(season) %>% 
...
office_ratings %>% 
  arrange(desc(imdb_rating)) %>% 
  head(5) %>% 
  kable(caption = "Top Five Episodes by IMDB Ratings",
        row.names = TRUE,
        col.names = c("Season", "Episode No.", "Title", "IMDB Rating", "Total Votes", "Air Date"),
